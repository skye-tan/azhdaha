...
long long find_product(LINEAR_TYPE long long *data1,
                       LINEAR_TYPE long long *data2, int size) {
    long long product = 1LL;
    for (int i = 0; i < size; i++) {
        product *= data1[i] * data2[i]; // Use after free
    }
    return product;
}
...
int main() {
    ...
    init_data(&d1, &d2, 18);
    process_data(d1, d2, 18);
    release_data(d1, d2);
    long long result = find_product(d1, d2, 18);
    return 0;
}